...
class JplagMatrixValueFactoryTest {
	...
	private static MatrixValueFactory<TestCaseMaterialization, BigDecimal> factory;

	@BeforeEach
	void setup() {
		this.factory = new JplagMatrixValueFactory();
	}
	...
	void same() throws Exception {
		...
		assertEquals(BigDecimal.ONE, this.factory.create(materialization, materialization));
	}
	...
	void notSame() throws Exception {
		...
		assertEquals(BigDecimal.ZERO, this.factory.create(materializationA, materializationB));
		assertEquals(BigDecimal.ZERO, this.factory.create(materializationB, materializationA));
	}

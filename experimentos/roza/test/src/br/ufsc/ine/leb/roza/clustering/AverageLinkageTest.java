...
class AverageLinkageTest {
	...
	void singleAndNonSingleElementClustersWithSymmetricSimilarity() throws Exception {
		...
		Linkage linkage = new AverageLinkage(builder.build());
		assertEquals(this.dotFour, linkage.evaluate(this.alphaBetaCluster, this.gammaCluster).stripTrailingZeros());
	}
	...
	void nonSingleElementClustersWithSymmetricSimilarity() throws Exception {
		...
		builder.add(this.beta, this.gamma, this.dotSix).add(this.beta, this.delta, this.dotEight);
		builder.add(this.gamma,this.delta, this.dotSeven);
		Linkage linkage = new AverageLinkage(builder.build());
		assertEquals(this.dotFive, linkage.evaluate(this.alphaBetaCluster, this.gammaDeltaCluster).stripTrailingZeros());
	}
	...
	void singleAndNonSingleElementClustersWithAsymmetricSimilarity() throws Exception {
		SimilarityReportBuilder builder = new SimilarityReportBuilder(false);
		builder.add(this.alpha, this.beta, this.dotEight).add(this.alpha, this.gamma, this.dotTwo);
		builder.add(this.beta, this.alpha, this.dotEight).add(this.beta, this.gamma, this.dotSix);
		builder.add(this.gamma, this.alpha, this.dotFour).add(this.gamma, this.beta, this.dotTwo);
		Linkage linkage = new AverageLinkage(builder.build());
	...
	void nonSingleElementClustersWithAsymmetricSimilarity() throws Exception {
		...
		builder.add(this.beta, this.alpha, this.dotEight).add(this.beta, this.gamma, this.dotSeven).add(this.beta, this.delta, this.dotTwo);
		builder.add(this.gamma, this.alpha, this.dotFour).add(this.gamma, this.beta, this.dotTwo).add(this.gamma, this.delta, this.dotEight);
		builder.add(this.delta, this.alpha, this.dotTwo).add(this.delta, this.beta, this.dotSix).add(this.delta, this.gamma, this.dotEight);
		Linkage linkage = new AverageLinkage(builder.build());
		assertEquals(this.dotFive, linkage.evaluate(this.alphaBetaCluster, this.gammaDeltaCluster).stripTrailingZeros());
	}

...
class ClusterJoinerWithSingleLinkageTest {
	...
	void linkSingleElementClusterWithNonSingleElementCluster() {
		...
		WinnerCombination winner = new ClusterJoiner(new SingleLinkage(report), this.referee).join(clusters);
	...
	void linkSingleElementClusters() {
		SimilarityReportBuilder builder = new SimilarityReportBuilder(true);
		builder.add(this.alpha, this.beta, this.dotTwo).add(this.alpha, this.gamma, this.dotEight).add(this.alpha, this.delta, this.dotOne);
		builder.add(this.beta, this.gamma, this.dotFive).add(this.beta, this.delta, this.dotSeven);
		builder.add(this.gamma, this.delta, this.dotThree);
		SimilarityReport report = builder.build();
		ClustersToMerge clusters = new ClustersToMerge(this.collectionUtils.set(this.alphaGammaCluster, this.betaCluster, this.deltaCluster));
		WinnerCombination winner = new ClusterJoiner(new SingleLinkage(report), referee).join(clusters);
		Combination combination = winner.getCombination();
	...
	void nonSymmetricAlphaBeta() {
		...
		builder.add(this.alpha, this.beta, this.dotSeven).add(this.alpha, this.gamma, this.dotEight);
		builder.add(this.beta, this.alpha, this.dotNine).add(this.beta, this.gamma, this.dotEight);
		builder.add(this.gamma, this.alpha, this.dotEight).add(this.gamma, this.beta, this.dotEight);
		SimilarityReport report = builder.build();
		...
		Assertions.assertEquals(this.dotNine, winner.getEvaluation());
	}

	@Test
	void nonSymmetricBetaAlpha() {
		SimilarityReportBuilder builder = new SimilarityReportBuilder(false);
		builder.add(this.alpha, this.beta, this.dotNine).add(this.alpha, this.gamma, this.dotEight);
		builder.add(this.beta, this.alpha, this.dotSeven).add(this.beta, this.gamma, this.dotEight);
		builder.add(this.gamma, this.alpha, this.dotEight).add(this.gamma, this.beta, this.dotEight);
		SimilarityReport report = builder.build();
		ClustersToMerge clusters = new ClustersToMerge(this.collectionUtils.set(this.alphaCluster, this.betaCluster, this.gammaCluster));
		WinnerCombination winner = new ClusterJoiner(new SingleLinkage(report), this.referee).join(clusters);
	...
	void multiplePossibilities() {
		...
		TiebreakException exception = assertThrows(TiebreakException.class, () -> {
			new ClusterJoiner(new SingleLinkage(report), this.referee).join(clusters);
		});

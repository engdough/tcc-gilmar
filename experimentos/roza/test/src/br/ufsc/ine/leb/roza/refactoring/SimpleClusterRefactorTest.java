...
public class SimpleClusterRefactorTest {
	...
	private static ClusterRefactor refactor;

	@BeforeEach
	void setup() {
		this.namingStrategy = new IncrementalTestClassNamingStrategy();
		this.refactor = new SimpleClusterRefactor(namingStrategy);
	}

	@Test
	void withoutClusters() throws Exception {
		List<TestClass> classes = this.refactor.refactor(new HashSet<>());
	...
	void oneClusterWithOneEmptyTest() throws Exception {
		...
		List<TestClass> classes = this.refactor.refactor(clusters);
